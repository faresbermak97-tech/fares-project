          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 mb-20">
            {results.map((result, index) => (
              ...
              >
                {/* Result Card */}
                <div 
                  className="relative bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-8 md:p-10 transition-all duration-500 hover:bg-white/10 hover:border-[#4D64FF]/50 hover:-translate-y-2 cursor-pointer"
                >
                  {/* Hover Glow Effect */}
                  ...
                  <div className="relative z-10">
                    ...
                    <div className="mt-4 flex items-center gap-2 text-sm text-[#4D64FF] opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                      ...
                      <svg className={`w-4 h-4 transition-transform ${hoveredResult === index ? 'rotate-180' : 'animate-bounce'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div className={`mt-4 bg-gradient-to-br from-gray-900 via-gray-800 to-black backdrop-blur-xl border-2 border-[#4D64FF]/50 rounded-2xl p-6 md:p-8 shadow-2xl transition-all duration-500 overflow-hidden ${
                  hoveredResult === index 
                    ? 'max-h-[500px] opacity-100' 
                    : 'max-h-0 opacity-0'
                }`}>
                  <div className="mb-6">
                    <h3 className="text-xl md:text-2xl font-bold text-[#4D64FF]">
                  ...
                  <div className="space-y-4">
                    {result.hoverData.details.map((detail, idx) => (
                      <div key={idx} className="flex items-start gap-3">
                        <span className="text-[#4D64FF] text-xl mt-1 flex-shrink-0">•</span>
                        <p className="text-gray-300 text-base leading-relaxed">{detail}</p>
                      </div>
                    ))}
                  </div>
                </div>
